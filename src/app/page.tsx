    ...
    <main className="min-h-screen bg-slate-50 text-slate-800 font-sans pb-20">
      <div className="max-w-5xl mx-auto p-6">
        ...
        <div className="text-center py-12 md:py-16 relative">
          ...
          {/* SYNC MODAL */}
          {showSync && (
            <div className="max-w-md mx-auto mb-8 bg-white p-6 rounded-2xl shadow-lg border border-emerald-100 animate-in fade-in slide-in-from-top-4">
              <h3 className="text-lg font-bold mb-2 text-slate-800">Sync this device</h3>
              ...
              <form onSubmit={handleSync} className="flex gap-2 flex-col">
                <input placeholder="Your Name" className="p-3 border rounded-xl bg-slate-50 focus:border-emerald-500 outline-none" value={syncName} onChange={e=>setSyncName(e.target.value)} required />
                <input placeholder="4-Digit PIN" className="p-3 border rounded-xl bg-slate-50 focus:border-emerald-500 outline-none" value={syncPin} onChange={e=>setSyncPin(e.target.value)} required maxLength={4} />
                <button disabled={loading} className="bg-emerald-600 hover:bg-emerald-700 text-white p-3 rounded-xl font-bold transition-colors">
